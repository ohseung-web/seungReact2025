...
export default function LoginNaver02() {
  ...
  // 로그인 화면을 보여줄지 여부 상태
  // savedIdFromCookie가 있으면 이미 로그인한 상태이므로 화면을 숨김
  const [showLogin, setShowLogin] = useState(!saveId); // 쿠키 있으면 로그인 화면 숨기기
  ...
  const loginClick = (e) => {
    ...
    if (idChk) {
      // 1분뒤 만료되는 쿠키
      const oneday = new Date(Date.now() + 24 * 60 * 1000);
      Cookie.set('userId', userId, { expires: oneday, path: '/' });
    } else {
      Cookie.remove('userId', { path: '/' });
    }
    alert(`로그인 시도 : ${userId}`);
    setShowLogin(false); // 로그인 성공 시 로그인 화면 숨김
  };
